...
%  -(a(x) u'(x))' = f(x) a.s., for x in [0,L],
% subject to u(0) = g a.s. and a(L)*u'(L) = h a.s. Here a(x) =a(w,x) is a 
% random field where the average (integral) over each element is supplied. 
% The force term f(x) is constant for x in [0,L]. 
%
...
function [U] = FEM1D(interval, numFemNodes, abar, force, condg, condh)                         
% *** INPUT ***
%
% FE MESH PROPERTIES
...
[~, elem] = generateMesh(interval,nn,ne);
dir = elem(1,1);                          
...
[K, Fe, Fg] = assembleGlobal(interval, elem, nn, abar, force, condg);
% Assemble F
F = Fe + Fh - Fg;
...
function [node, connectivity] = generateMesh(interval,numNodes,numElements)
l = interval(1);
L = interval(2);
...
function [K, Fe, Fd] = assembleGlobal(interval, elem, nn, a, f, g)
